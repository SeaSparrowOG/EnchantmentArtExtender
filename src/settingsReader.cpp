...
namespace Settings {
	...
	ErrorReport Settings::IsConfigValid(std::string config, Json::Value a_JSON) {
		...
		for (const auto& field : requiredFieldsAtLeastOne) {
			const auto& found = a_JSON[field];
			if (found)
				foundOne = true;
		}
		...
		if (a_JSON["Enchantment"]) {
			const auto& allFields = a_JSON["Enchantment"];
			if (allFields.size() != 2) {
				response.valid = false;
				response.foundGarbage.push_back("Enchantment");
			}
			else {
				const auto& idRaw = a_JSON["Enchantment"]["ID"];
				const auto& sourceRaw = a_JSON["Enchantment"]["Source"];
				if (!(idRaw && sourceRaw)) {
					response.valid = false;
					response.foundGarbage.push_back("Enchantment");
				}
		...
		const auto& allFields = a_JSON.getMemberNames();
		for (const auto& field : allFields) {
			bool fieldIsValid = false;
			for (const auto& expectedField : expectedFields) {
		...
		for (const auto& entry : expectedString) {
			const auto& field = a_JSON[entry];
			if (!field) continue;
		...
		for (const auto& entry : expectedList) {
			const auto& field = a_JSON[entry];
			if (!field) continue;
		...
		for (const auto& entry : expectedBool) {
			const auto& field = a_JSON[entry];
			if (!field) continue;
		...
		for (const auto& entry : expectedTopLevelObject) {
			const auto& field = a_JSON[entry];
			if (!field) continue;
		...
		if (a_JSON["SwapData"] && a_JSON["SwapData"].isArray()) {
			for (const auto& data : a_JSON["SwapData"]) {
				if (!data.isObject()) {
					response.valid = false;
				...
				const auto& allDataNames = data.getMemberNames();
				...
				const auto& swapName = allDataNames.at(0);
				const auto& swapInformation = data[swapName];
				...
				const auto& informationNames = swapInformation.getMemberNames();
				int matches = 0;
				...
				matches = 0;
				for (const auto& name : informationNames) {
					for (const auto& field : expectedInformationAtLeastOne) {
